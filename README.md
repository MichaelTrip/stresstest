# Stresstest generator

![Author](https://img.shields.io/badge/guru-MichaelTrip-3399cc.svg?logo=data:image/svg+xml;base64,)

![Build status](https://github.com/MichaelTrip/stresstest/actions/workflows/main.yaml/badge.svg)
![GitHub License](https://img.shields.io/github/license/michaeltrip/stresstest)
![GitHub last commit](https://img.shields.io/github/last-commit/michaeltrip/stresstest?logo=github)
![GitHub tag (with filter)](https://img.shields.io/github/v/tag/michaeltrip/stresstest)


## README

A simple load generator to stresstest your Kubernetes cluster.

## How to use?

1. download the `deployment.yml` from this repo
2. run `kubectl apply -f deployment.yml`
3. scale your replicas `kubectl scale deployment stresstest --replicas <10>`
4. Whats the behaviour.
5. To scale down: `kubectl scale deployment stresstest --replicas=1`
6. To delete: `kubectl delete -f deployment.yml`
